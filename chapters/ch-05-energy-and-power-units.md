...
::::{admonition} Definition 5.1.1 — Energy
:class: important
:label: def-5-1-1

is defined as the capacity to do work. Work is well-defined in physics as the **application of force through a distance**.[^1] The colloquial use of the word “work” matches relatively well, in that pushing a large couch across the floor (applying force through a distance) or lifting a heavy box up to a shelf feels like work and can tire you out.

::::

The SI unit of force is the Newton (N), breaking down more fundamentally to kg $\cdot \mathrm{m/s}^{2}$. The best way to remember this is via Newton’s Second Law: $F = ma$ (force equals mass times acceleration). Mass has units of kg, and acceleration[^2] is measured in meters per second squared.
...
Joule (J). Thus, the application of 1 N of force across a distance of 1 m constitutes 1 J of work, requiring 1 J of energy to perform. [Table 5.1](#tab-5-1) offers contextual examples (unit prefixes are on page 433).

:::{table} Approximate energy for familiar activities. The first freeway example is just kinetic energy; the second is the energy cost of a whole trip.
:label: tab-5-1
:enumerator: 5.1
...
Writing out Newtons as kg $\cdot \mathrm{m/s}^{2}$, we find that the unit of energy amounts to J $=$ N $\cdot$ m $=$ kg $\cdot \mathrm{m}^{2}/\mathrm{s}^{2}$. Notice that this looks like mass times velocity-squared. [Box 5.1](#box-5-1) explores how this makes a lot of sense.

::::{admonition} Box 5.1: The Units Make Sense!
:class: tip
:label: box-5-1
...
:::{margin}
More on mass-energy in [Chapter 15](#ch-15).

:::

:::{margin}
More on kinetic energy in [Chapter 12](#ch-12).
...
:::{margin}
[Sec. A.10](#sec-a-10) (p. 382) in the Appendices provides additional guidance on manipulating units.
...
Energy manifests in a variety of forms, which we will treat in greater detail in application-specific chapters in Part III of this text. For now we just want to name them and point to related chapters and applications, as is done in [Table 5.2](#tab-5-2).

:::{table} Energy forms. Exchange is possible between all forms. Chemical energy is represented here by Gibbs free energy.
:label: tab-5-2
:enumerator: 5.2
...
::::{admonition} Box 5.2: Energy: The Money of Physics
:class: tip
:label: box-5-2
...
[Example 5.2.1](#ex-5-2-1) traces a few familiar energy conversions, and [Figure 5.1](#fig-5-1) provides an example illustration. A more encompassing narrative connecting cosmic sources to daily use is provided in [Sec. D.2.2](#sec-d-2-2) (p. 408).

::::{admonition} Example 5.2.1
:class: seealso
:label: ex-5-2-1
...
::::{admonition} Definition 5.3.1 — Power
:class: important
:label: def-5-3-1

is simply defined as energy per time: how much energy is expended in how much time. The SI unit is therefore J/s, which we rename **Watts** (W).
...
While energy is the capacity to do work, it says nothing about how quickly that work might be accomplished. Power addresses the *rate* at which energy is expended. [Figure 5.2](#fig-5-2) provides a sense of typical power levels of familiar animals and appliances.

::::{admonition} Example 5.3.1
:class: seealso
:label: ex-5-3-1
...
Of course, we commonly apply the usual multipliers of factors of $10^{3}$ to the unit to make it more useful. Thus we have the progression W, kW, MW, GW, TW, etc. For reference, a large college campus will require several tens of MW (megawatts) for electricity. A large power plant is typically in the 1–4 GW range. See [Table 5.3](#tab-5-3) for scales at which we are likely to use the various multiplying factors, and a more complete set of multipliers on page 433.

:::{table} Power multipliers and contexts
:label: tab-5-3
:enumerator: 5.3
...
::::{admonition} Definition 5.4.1
:class: important
:label: def-5-4-1

The **kilowatt-hour** is an amount of energy (not a power) resulting from an expenditure of energy at a rate of 1 kW for a duration of one hour, and is the unit of choice for residential electricity usage.
...
:::{margin}
It may help to think of the sequence: kilowatt $\times$ hour; $\mathrm{kW}\times$h; kW-h; kWh.

:::

::::{admonition} Example 5.4.1
:class: seealso
:label: ex-5-4-1
...
::::{admonition} Box 5.3: Don’t be one of those people...
:class: tip
:label: box-5-3
...
::::{admonition} Example 5.4.2
:class: seealso
:label: ex-5-4-2

We will explore kWh using a light bulb for an example. Let’s say the light bulb is labeled as 100 W.[^12] How much energy does it use?
...
:::{margin}
This might win the prize for the dumbest convention in science: never define a unit as case-sensitive, as it cannot be differentiated in spoken language!
:::
...
::::{admonition} Example 5.5.1
:class: seealso
:label: ex-5-5-1

To change 30 mL (30 g) of water by $5^{\circ}\mathrm{C}$ requires 150 cal, or a little over 600 J.
...
Notice that we did not apply an explicit formula in [Example 5.5.3](#ex-5-5-3). By proceeding stepwise, we attempt to keep it intuitive. We *could* write a

:::{margin}
Appendix [Sec. A.8](#sec-a-8) (p. 380) addresses this philosophy in a bit more detail.
...
:::{margin}
**Try it:** This would be an excellent opportunity to *create your own* formula to capture the idea, like an expert!
...
::::{admonition} Definition 5.6.1
:class: important
:label: def-5-6-1

The **Btu** is the Imperial analog to the kcal.[^19] One Btu is the energy required to heat one pound of water one degree Fahrenheit.
...
The U.S. uses quadrillion Btu to represent country-scale annual energy expenditures. It is denoted as qBtu, or informally “quads.” One qBtu is approximately $10^{18}$ J.[^20]
...
::::{admonition} Example 5.6.1
:class: seealso
:label: ex-5-6-1

For appliances characterized by Btu/hr, we can relate to power in Watts via 1 Btu/hr as 1,055 J per 3,600 s, working out to 0.293 W.
...
1. Heating 15 gallons[^22] (125 pounds) from a cool $68^{\circ}$F to a hot $131^{\circ}$F at 30,000 Btu/hr will take how long? We must put in
...
2. In metric terms, the equivalent to 15 gallons is 57 L (57 kg), and we heat from $20^{\circ}\mathrm{C}$ to $55^{\circ}\mathrm{C}$ at 8,800 W.[^23] Since one kcal heats one kilogram of water $1^{\circ}\mathrm{C}$, heating 57 kg by $35^{\circ}\mathrm{C}$ will require 57 $\times 35$ kcal, or 57 $\times 35 \times 4,$ 184 J $= 8.35$ MJ, which at 8,800 W will take 950 seconds, also just over 15 minutes (reassuringly, the same answer).
...
::::{admonition} Definition 5.7.1
:class: important
:label: def-5-7-1

One **Therm** is 100,000 Btu, or $1.055 \times 10^{8}$ J, or 29.3 kWh.

::::

::::{admonition} Box 5.5: Why Therms?
:class: tip
:label: box-5-5

The Therm is partly adopted for the near-convenience that 100 cubic feet of natural gas (CCF or 100 CF), which meters measure directly, equates to 1.036 Therms. Relatedly, one gallon (3.785 L) of liquid propane gas[^26] contains 91,500 Btu, which is 0.915 Therms. Thus the Therm very closely matches convenient measures of natural gas (100 cubic feet) or liquid propane (a gallon).

::::

::::{admonition} Example 5.7.1
:class: seealso
:label: ex-5-7-1

It might take approximately 10,000 kcal of energy[^27] to heat a fresh infusion of cold water into a hot water heater tank. How many Therms is this?
...
:::{margin}
It is interesting to reflect on the notion that 200 L of water can be heated by $50^{\circ}\mathrm{C}$ for only 2 L of liquid fuel: 1% of the water volume in fuel. If heating to boiling, it would take twice as much fuel, so 2% of the water volume. Seems like a good bargain— especially for backpackers who want to boil water and have to lug the fuel around to do so. Inefficiencies in getting heat into the water might require more like 10% fuel volume.
...
::::{admonition} Definition 5.8.1
:class: important
:label: def-5-8-1

The amount of energy in a charge, $q$, at a voltage, $V$, is
...
Current is the rate at which charge flows, and is usually symbolized by the letter $I$. Imagine setting up a toll booth in a conducting wire and counting how many charges (or how much cumulative charge) pass the gate per unit time. This gives rise to the [Definition 5.8.2](#def-5-8-2).

::::{admonition} Definition 5.8.2
:class: important
:label: def-5-8-2

Current is measured in Amps,[^29] which is defined as one Coulomb per second.

::::

Moving one Coulomb through one Volt every second would constitute one Joule of energy every second, which is the definition of one Watt. Putting the concepts of [Definition 5.8.1](#def-5-8-1) and [Definition 5.8.2](#def-5-8-2) together, we find ourselves able to define electrical power.

::::{admonition} Definition 5.8.3
:class: important
:label: def-5-8-3

Electrical power is simply current multiplied by voltage:
...
::::{admonition} Example 5.8.1
:class: seealso
:label: ex-5-8-1

Households in the U.S. often have circuit breakers allowing maximum currents of 15 or 20 Amps for regular power outlets. At a voltage of 120 V,[^30] this corresponds to a maximum power of 1,800 W or 2,400 W, respectively.[^31]
...
A typical 9-volt battery has a capacity of 500 mAh. How much energy is this?
...
We have already hit all the relevant concepts for understanding the eV in [Section 5.8](#sec-5-8). The main reason to have its own section is so that it appears separately in the table of contents, making it easier to find and reference. The definition follows [Definition 5.8.1](#def-5-8-1) closely.

::::{admonition} Definition 5.9.1
:class: important
:label: def-5-9-1

One **electron-volt** is the energy associated with pushing one fundamental unit of electron charge, $|e| = 1.6\times 10^{-19}$ Coulombs, through an electric potential of 1 V:

:::{math}
:label: eq-5-3
:enumerator: 5.3
1 eV = 1.6 \times 10^{-19} C \cdot 1 V = 1.6 \times 10^{-19} J
:::
...
::::{admonition} Example 5.9.1
:class: seealso
:label: ex-5-9-1

When 12 grams of carbon (one mole, or $6\times 10^{23}$ atoms[^35]) reacts with oxygen to form CO$_{2}$, about 394 kJ of energy is released.[^36] How much energy is this *per carbon atom* in electron-volts?

Since we have one mole, or 6 $\times 10^{23}$ carbon atoms, we divide our total energy $(3.94 \times 10^{5}$ J) by the number of atoms to get $6.5 \times 10^{-19}$ J per atom. This is just a bit larger than 1 eV $(1.6 \times 10^{-19}$ J), and the division leads to something very close to 4 eV per atom.

Because CO$_{2}$ has a total of four bonds between the carbon atom and the two oxygen atoms,[^37] we see that each bond accounts for about 1 eV. Chemical bonds are often in this range, highlighting the usefulness of the eV unit at the atomic level.
...
Light energy and its spectrum will be explored more extensively in [Chapter 13](#ch-13), but the main concepts are covered here for completeness.
...
::::{admonition} Example 5.10.1
:class: seealso
:label: ex-5-10-1

Visible light has a wavelength of 0.4–0.7 $\mu \mathrm{m}$,[^39] corresponding to 2.8–$5.0\times 10^{-19}$ J for each photon.

::::

We also routinely express photon energy in electron-volts (eV) according to [Definition 5.10.2](#def-5-10-2).

::::{admonition} Definition 5.10.2
:class: important
:label: def-5-10-2

Given the wavelength in microns $(\mu \mathrm{m})$, the energy of a photon in eV units is
...
::::{admonition} Example 5.10.2
:class: seealso
:label: ex-5-10-2

The red-end of the visible spectrum, around 0.7 $\mu \mathrm{m}$, corresponds to photon energies around 1.8 eV, while the blue-end, around 0.4 $\mu \mathrm{m}$, corresponds to 3.1 eV.
...
:::{margin}
Weight is $mg$, where $g \approx 10 \mathrm{m/s}^{2}$.
...
10. If a 70 kg person (weight: 700 N) is capable of putting out energy at a rate of 500 W in short bursts, how long will it take the person to race up a flight of stairs 4 m high, considering only the vertical energy[^42] required?
...
12. How many kcal will it take to heat 1 liter of water (e.g., in a pot) from room temperature $(20^{\circ}\mathrm{C})$ to boiling $(100^{\circ}\mathrm{C})$? How many Joules is this?

13. If a microwave operates at a power of 1,600 W (1,600 J/s), how long will it take to heat 0.25 L of water from room temperature to boiling (changing temperature by $80^{\circ}\mathrm{C})$ if 50% of the microwave energy is absorbed by the water?
...
:::{margin}
ⓘ The result can help inform your sense for the typical range of human metabolic power.
...
20. The chapter banner image (page 73) shows data from the author’s utility bill, indicating 230 kWh of electrical usage for a 30-day period in 2020. What does this rate of energy usage translate to, in Watts?
...
23. If Problem 21 had resulted in 5,000 W,[^46] how many kilowatt-hours (kWh) are expended in a 24-hour period? At an electricity cost of around \$0.15 per kWh,[^47] about how much will it cost, per day, to maintain heat?
...
26. The chapter banner image (page 73) shows data from the author’s utility bill, reflecting 230 kWh of electricity and 4 Therms of gas usage. Annoyingly, the units are different. How do the actual energies compare, if expressed in the same units?[^51] How would you capture in a simple sentence the approximate comparison of energy use for each?

27. The chapter banner image (page 73) shows part of the hot water heater label in the author’s home, showing a rating of 40,000 Btu/hr. How much power is it capable of putting out, in Watts?

28. The chapter banner image (page 73) shows the energy label associated with the author’s hot water heater, estimating that it will use 242 Therms per year. If the estimated energy cost is distributed evenly across 12 months, what would the utility bill be expected to report? Based on an actual utility bill in the same image, the usage for one billing period was 4 Therms. How does actual usage compare to estimated usage, as an approximate percentage?

29. The chapter banner image (page 73) has two panels relating to the same hot water heater. One indicates the rate of gas usage when the heater is on (ignited, heating water) as 40,000 Btu/hr, and the other anticipates 242 Therms per year will be used. How many hours per day is the heater expected to be on (heating water) based on these numbers?
...
34. The chapter banner image (page 73) shows a rechargeable AA battery, operating at 1.2 V and holding 2,200 mAh of charge. How many Joules is this, and how long could it power a 1 W LED array?

35. If we have 6 $\times 10^{23}$ molecules,[^53] and each molecule releases 1 eV in a chemical reaction, how many kJ (per mole, as it turns out) is this reaction?
...
[^19]: Recall that 1 kcal is the energy it takes to heat one kilogram of water by $1^{\circ}\mathrm{C}$.
...
[^24]: See, for instance, the banner image for this chapter on page 73.
[^25]: [Chapter 20](#ch-20) will explore what might be learned from utility bills.
...
[^27]: Based on a capacity of 200 L, pulling in chilly water at $5^{\circ}\mathrm{C}$ and heating it to $55^{\circ}\mathrm{C}$, thereby requiring 200 kg $\times 50$ C kcal.
...
[^31]: Safety regulations limit continuous use to 80% of the breaker current capacity, so that realistically the limits are 1,400 W and 1,920 W, respectively. This is why “heating appliances” in [Figure 5.2](#fig-5-2) top out around 1,500 W: circuit/safety limits.
...
[^35]: See [Appendix B](#app-b) for a primer/refresher on chemistry.
...
[^46]: ⚠ it does not, exactly
[^47]: … regionally variable
[^48]: ⚠ it does not, exactly
...
[^53]: This is one mole, as covered in [Sec. B.1](#sec-b-1) (p. 387).
